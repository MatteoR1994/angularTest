...
describe('PaperinoService', () => {
  ...
  it('#getValue should return stubbed value from a spy', () => {
    // create `getValue` spy on an object representing the PlutoService
    const plutoServiceSpy = jasmine.createSpyObj('PlutoService', ['getValue']);
    ...
    expect(service.getValue()).withContext('service returned stub value').toBe(stubValue.toUpperCase());
    expect(plutoServiceSpy.getValue.calls.count()).withContext('spy method was called once').toBe(1);
    expect(plutoServiceSpy.getValue.calls.mostRecent().returnValue).toBe(stubValue);
  });
});
